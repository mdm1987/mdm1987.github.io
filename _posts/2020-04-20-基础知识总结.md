...
#####分布式事务，两阶段提交。
	...
	数据不一致 在提交阶段，当协调者向参与者发送commit请求之后，发生了局部网络异常或者在发送commit请求过程中协调者发生了故障，这会导致只有一部分参与者接受到了commit请求。而在这部分参与者接到commit请求之后就会执行commit操作。但是其他部分未接到commit请求的机器则无法执行事务提交。于是整个分布式系统便出现了数据不一致性的现象。二阶段提交协议没有设计较为完善的容错机制，任意一个节点的失败都会导致整个事务的失败
	三阶段提交：
	...
	总结：2PC 简单，锁定资源全部在第一阶段perpare，二是没有超时机制，会导致资源锁定阻塞，二是数据不一致的问题。3PC 将第一阶段一拆为二的目的也就是延迟锁定资源的时期，先确定资源，第二阶段perpare才锁定资源，并且加入了超时机制，参与者超时，默认执行commit
